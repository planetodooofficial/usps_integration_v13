...
class UPSRefundRequest(UPSShipping):
    ...
    def _get_data(self):
        ...
        data.append(
            'https://wwwcie.ups.com/ups.app/xml/Void' if self.ups_info.test else 'https://onlinetools.ups.com/ups.app/xml/Void')
        return data
...
class UPSRateRequest(UPSShipping):
    ...
    def __init__(self, ups_info, pickup_type_ups, service_type_ups, packaging_type_ups, weight, shipper, receipient,
                 cust_default, sys_default, measurement, residential):
        self.type = 'UPS'
        self.ups_info = ups_info
...
class UPSRateResponse(object):
    def __init__(self, root, weight, cust_default, sys_default):
        ...
        sr_no = 2 if sr_no == 9 and sys_default and sys_default.split('/')[0] == self.type and sys_default.split('/')[
            1] == mail_service else sr_no
        self.postage = [{'Rate': root.findtext('RatedShipment/TotalCharges/MonetaryValue'), 'Service': mail_service,
                         'sr_no': sr_no}]
